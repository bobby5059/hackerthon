...
### shared 정합성 조정 (2026-08-31, rebase 후 반영)
main의 `shared` 유닛(FD+NFR, PR #1~#2)과 rebase하며 개념 정합성을 검토하고, **shared 우선** 원칙으로 아래를 조정(사용자 확정):
- **Q10 → A로 정정(메뉴 가용성 미도입)**: `Menu.is_available` 필드·품절 422 규칙(구 BR-ORD-05)·`MENU_UNAVAILABLE` 코드 제거. 근거: 계약 v1.0 §3 및 `shared` 미러 타입에 없고, 요구사항 §5에서 재고 관리가 제외 범위. 계약 §3.2와 완전 일치, 계약 §9 변경 불필요.
- **409 에러 코드 = `SESSION_CLOSED` 재사용**: 재-setup(Q5=C)·미완료 이용완료(Q2=B) 충돌에 별도 신규 코드 대신 계약 정의 코드 사용(계약 코드 집합 불변).
- **`has_new` 항상 `false`로 포함(생략 금지)**: `shared` TableCard가 필수 필드로 요구하므로 응답에 항상 포함. 서버 신규 계산은 여전히 없음(Q9=A).
